...
static char std_output_buffer[BUFFER_SIZE] = {0};
static char std_error_buffer[BUFFER_SIZE] = {0};
static char output_name[] = "tmp/NUL";

void replace_stdout()
{
    fflush(stdout); //clean everything first
    stdout_save = dup(STDOUT_FILENO); //save the stdout state
    freopen(output_name, "a", stdout); //redirect stdout to null pointer
    setvbuf(stdout, std_output_buffer, _IOFBF, BUFFER_SIZE); //set buffer to stdout
}

void restore_stdout()
{
    freopen(output_name, "a", stdout); //redirect stdout to null again
...
void replace_stderr()
{
    ...
    freopen(output_name, "a", stderr); //redirect stderr to null pointer
    setvbuf(stderr, std_error_buffer, _IOFBF, BUFFER_SIZE); //set buffer to stderr
}

void restore_stderr()
{
    freopen(output_name, "a", stderr); //redirect stderr to null again
